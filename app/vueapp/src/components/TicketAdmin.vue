<template>
  <v-card class="no-margin">
    ...
    <v-card-actions>
      <template v-if="!['close', 'rejected'].includes(data.status__value)">
        <v-btn flat color="orange" :disabled="select === null" @click.native="setSolver">Set solver</v-btn>
        <v-dialog
...
<script>
  ...
  module.exports = {
    ...
    props: ['data', 'parent', 'dialog', 'solvers'],
    ...
    computed: {
      solversArr: function () {
        return this.solvers.map((solver) => solver.full_name + ' (' + solver.user__username + ')')
      }
    }
  }
